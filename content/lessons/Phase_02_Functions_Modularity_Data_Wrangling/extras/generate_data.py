...
def generate_dirty_customer_data(
    output_path: str = "dirty_customer_data.csv", n: int = 200
):
    ...
    for i in range(n):
        ...
        phone = (
            ...
            else f"555-{random.randint(100, 999)}-{random.randint(1000, 9999)}"
        )
        rows.append(
            {
                "Name": name,
                ...
                "City": random.choice(
                    ["Mumbai", "delhi", "BANGALORE", "  Chennai  ", None]
                ),
                "Revenue": round(random.gauss(5000, 2000), 2)
