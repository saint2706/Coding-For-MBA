...
- **Hyperparameter**: A configuration value set before training that controls the learning process itself (e.g., `learning_rate`, `max_depth`, `alpha`), as opposed to model parameters (weights) that are learned from data.
- **Search space**: The set of all possible hyperparameter values or distributions to be explored during tuning; poorly defined search spaces lead to wasted computation or missed optima.
- **Grid search**: An exhaustive hyperparameter tuning method that evaluates every combination of a predefined set of values, guaranteeing coverage but scaling exponentially with the number of parameters.
- **Random search**: A hyperparameter tuning method that samples parameter combinations randomly from specified distributions; it often matches or beats grid search quality at a fraction of the computational cost, especially in high-dimensional spaces.
- **Bayesian optimization**: An intelligent sequential tuning strategy that builds a probabilistic surrogate model of the objective function and uses it to select the most promising hyperparameter configurations to evaluate next, outperforming random search with limited budgets.
- **Recursive Feature Elimination (RFE)**: A wrapper-based feature selection method that repeatedly trains a model, removes the least important feature(s), and repeats until the desired number of features remains; `RFECV` adds cross-validation to find the optimal count automatically.
- **Permutation importance**: A model-agnostic feature importance measure that estimates each feature's contribution by randomly shuffling its values and measuring the resulting drop in model performance on a held-out set.
- **AutoML**: Automated Machine Learning; frameworks (e.g., TPOT, Auto-sklearn, H2O) that automate the end-to-end process of algorithm selection, feature engineering, and hyperparameter tuning using meta-learning or evolutionary strategies.

---

## Cross-References

- **Day 45** — Feature engineering: the upstream process of creating informative features that complements the feature selection methods (RFE, permutation importance, Lasso) covered in this lesson.
- **Day 52** — Ensemble methods: the primary model family whose hyperparameters (`n_estimators`, `max_depth`, `learning_rate`, sampling rates) are systematically tuned using the techniques in this lesson.
- **Day 54** — Probabilistic modeling: well-tuned models still require probability calibration, which is the next step after hyperparameter optimization for risk-sensitive applications.
- **Day 50** — MLOps fundamentals: experiment tracking with MLflow is the production tool for logging, comparing, and auditing the many tuning runs generated by grid, random, and Bayesian search.
