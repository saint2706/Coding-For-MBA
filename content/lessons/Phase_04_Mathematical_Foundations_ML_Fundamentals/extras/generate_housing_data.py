...
def generate_housing_csv(n=200, seed=42):
    ...
    for _ in range(n):
        ...
        if neighborhood == "Waterfront": base *= 1.3
        elif neighborhood == "Downtown": base *= 1.1
        elif neighborhood == "Rural": base *= 0.8
        noise = random.gauss(0, base * 0.05)
        price = max(50000, int(base + noise))
        rows.append(f"{sqft},{beds},{baths},{age},{garage},{pool},{neighborhood},{price}")
    return "\n".join(rows)
