...
-- Includes intentional design choices to discuss in class (normalization, indexing).
...
-- Exercise 3: Calculate the 7-day rolling average revenue per customer segment
-- (Hint: SUM() OVER (PARTITION BY segment ORDER BY order_date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW))
...
-- EXPLAIN ANALYZE SELECT ... WHERE customer_id = X AND order_date >= NOW() - INTERVAL '90 days';
