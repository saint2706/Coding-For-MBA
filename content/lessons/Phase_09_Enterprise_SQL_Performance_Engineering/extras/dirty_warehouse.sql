...
-- A deliberately messy data warehouse scenario for performance tuning exercises.
...
-- SELECT * FROM raw_events WHERE user_id = 12345 ORDER BY event_date DESC LIMIT 50;
-- SELECT * FROM raw_events WHERE event_type = 'purchase' AND event_date >= NOW() - INTERVAL '7 days';
...
-- For each of these 3 query patterns, argue whether B-Tree, partial, or BRIN is best:
--   A. WHERE user_id = ? ORDER BY event_date DESC LIMIT 50
--   B. WHERE event_type = 'purchase' AND event_date >= NOW() - INTERVAL '7 days'
...
--   - Traverses all 5 levels of hierarchy
--   - Returns: name, level, salary, cumulative_subtree_cost (salary sum of entire subtree below each node)
...
-- Demonstrate: SET app.tenant_id = '...'; SELECT * FROM tenant_reports; -- should only see own rows
-- Check EXPLAIN to verify index is being used

-- Exercise 6: Audit Trigger
-- Create audit_log table with: log_id, table_name, action, old_credit_score, new_credit_score, changed_by, changed_at
...
-- Run EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT) on each of the following and identify the bottleneck:
