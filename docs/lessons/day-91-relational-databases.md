...
- **Example:** In the `employees` table, columns might include `employee_id`, `first_name`, `last_name`, and `hire_date`.
...
- **Example:** If we have a `departments` table with a primary key `department_id`, we can add a `department_id` column to our `employees` table to specify which department each employee belongs to. This `department_id` in the `employees` table is a foreign key.
