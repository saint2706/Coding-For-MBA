...
- Differentiate operating model topics such as BI operations, stakeholder identification, and key
  business functions.
...
1. **Operating model orientation** – Facilitate a discussion on how BI teams structure the work,
   using the roadmap topics as anchors.
1. **Stakeholder mapping exercise** – Break cohorts into pairs to match stakeholder personas with
   the BI operations that unlock their outcomes.
1. **Strategy tier decisions** – Lead a dialogue about when to apply operational vs tactical vs
   strategic BI and what artifacts each requires.
1. **Action planning** – Summarize the commitments and metrics each stakeholder group should own
   after the session.
...
- Close the session with a commitment from each stakeholder persona: what decision will they make
  differently because of the BI strategy?
