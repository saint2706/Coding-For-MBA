...
export default function ConceptGraph({ search = '', highlightPhase = null }: ConceptGraphProps) {
    ...
    useEffect(() => {
        ...
        const nodeElements = nodeG.selectAll<SVGGElement, GraphNode>('g')
            .data(nodes, (d) => d.id)
            .join('g')
        ...
        const simulation = d3.forceSimulation<GraphNode>(nodes)
            ...
            .force('cluster', d3.forceX<GraphNode>((d) => clusterCenters.get(d.phase)?.x ?? width / 2).strength(0.15))
            .force('clusterY', d3.forceY<GraphNode>((d) => clusterCenters.get(d.phase)?.y ?? height / 2).strength(0.15))
            .alphaDecay(0.02)
            .velocityDecay(0.4)
    ...
    useEffect(() => {
        ...
        function checkNodeMatch(nodeId: number): boolean {
            ...
            if (!svg) return false
            let match = false
            d3.select(svg).selectAll<SVGGElement, GraphNode>('.graph-node').each(function (d) {
                if (d.id === nodeId) {
                    const matchSearch = !search ||
                        d.label.toLowerCase().includes(searchLower) ||
                        d.title.toLowerCase().includes(searchLower) ||
                    ...
                    {tooltip.node.concepts.length > 0 && (
                        <div className="graph-tooltip-concepts">
                            {tooltip.node.concepts.slice(0, 5).map((c) => (
                                <span key={c} className="graph-tooltip-concept">{c}</span>
                            ))}
                        </div>
                    )}
